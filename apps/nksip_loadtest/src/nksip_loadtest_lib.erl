...
start_server(Port) ->
    ...
    case nksip:start_link(nksip_loadtest_server, Opts) of
        {ok, _} ->
            ok;
...
stop_server() ->
    nksip:stop(nksip_loadtest_server).
    ...
    MsgType = case lists:member(invite, Opts) of
        ...
        false ->
            case lists:member(register, Opts) of
                true -> 
                    nksip_registrar:clear(nksip_loadtest_server),
                    register;
                _ -> 
                    options
            end
                ...
                case Transport of
                    ...
                    tls ->
                        TlsOpts = [binary, {active, false}] ++
                            nkpacket_tls:make_outbound_opts(#{}),
...
start_clients(Pos, Max) ->
    Opts = #{
        ...
        callback_module => nksip_loadtest_client
    },
    case nksip:start_link(get_client(Pos), Opts) of
        {ok, _} ->
...
iter_full(MsgType, Pos, RUri, Pid, CallId0, Messages) ->
    ...
    Ok = try
        case MsgType of
            options -> 
                case nksip_uac:options(get_client(Pos), RUri, Opts) of
                    {ok, 200, []} ->
                        ok;
                    Other ->
            ...
            register ->
                ...
                case nksip_uac:register(get_client(Pos), RUri, Opts1) of
                    {ok, 200, []} ->
                        ok;
                    Other ->
            ...
            invite ->
                case nksip_uac:invite(get_client(Pos), RUri, Opts) of
                    {ok, 200, [{dialog, D}]} -> 
...
store_clients(Max) ->
    lists:foreach(
        fun(Pos) ->
            Atom = list_to_atom("nksip_loadtest_client_"++integer_to_list(Pos)),
            nklib_util:do_config_put({nksip_test_client, Pos}, Atom)
        end,
        lists:seq(1, Max)).


get_client(Pos) ->
    nklib_util:do_config_get({nksip_test_client, Pos}).
