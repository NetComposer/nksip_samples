...
-export([start_server/0, launch/1, full/0, full/1]).
...
full(Opts) ->
    nklib_log:console_loglevel(warning),
...
% 'Only UAS' set of tests use the "raw" version of the load suite
% UDP can't be used in raw mode because raw clients won't retransmit
