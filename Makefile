...
tutorial:
	ERL_AFLAGS=$(AFLAGS) $(REBAR) shell --name $(APP)@127.0.0.1 --config config/shell.config --setcookie nk --apps nksip_tutorial


pbx:
	ERL_AFLAGS=$(AFLAGS) $(REBAR) shell --name $(APP)@127.0.0.1 --config config/shell.config --setcookie nk --apps nksip_pbx

shell:
	ERL_AFLAGS=$(AFLAGS) $(REBAR) shell --name $(APP)@127.0.0.1 --config config/shell.config --setcookie nk --apps nksip
